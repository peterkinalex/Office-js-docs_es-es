...
# Usar PHP para validar un token de identidad
...
1. Extraiga el JWT de una cadena con codificación URL base64.
    
2. Asegúrese de que el token está bien formado, que es para su complemento de Outlook, que no ha expirado y que puede extraer una dirección URL válida para el documento de metadatos de autenticación.
    
3. Recupere el documento de metadatos de autenticación del servidor Exchange y valide la firma adjunta al token.
    
4. Calcule un identificador único para el usuario mediante el cifrado id. de Exchange del usuario con la dirección URL del documento de metadatos de autenticación.
    
Aunque el proceso pueda parecer complejo en conjunto, cada paso individual es bastante sencillo. Puede ver que la biblioteca de PHP interrumpe el proceso de esta forma exactamente al examinar el código para la función válida.
...
## Biblioteca de validación de token de identidad

El siguiente código es una biblioteca de PHP para validar un token de identidad de Exchange.
