...
Para que la eliminación de la propiedad sea permanente, debe llamar al método [`saveAsync`](#saveasynccallback-asynccontext) del objeto `CustomProperties`.
...
Debe llamar al método `saveAsync` para almacenar cualquier cambio realizado con el método [`set`](CustomProperties.md#set) o el método [`remove`](#removename) del objeto `CustomProperties`. La acción de guardado es asincrónica.
...
El método `set` configura la propiedad especificada al valor especificado. Debe usar el método [`saveAsync`](#saveasynccallback-asynccontext) para guardar la propiedad en el servidor.
